...
Gere as correções necessárias em JSON.`;
  ...
  // Rota de alterações: interpreta e aplica alterações do cliente
  if (req.method === 'POST' && req.url === '/alteracoes') {
    let body = '';
    req.on('data', c => body += c);
    req.on('end', async () => {
      try {
        const { alteracoesTexto, jsonData, clientName } = JSON.parse(body);
        if (!alteracoesTexto) throw new Error('Texto de alterações obrigatório');
        if (!jsonData) throw new Error('JSON da página obrigatório');

        const parsed = typeof jsonData === 'string' ? JSON.parse(jsonData) : jsonData;
        const texts = extractElementorTexts(parsed);

        // Passo 1: IA interpreta as alterações e gera lista estruturada
        const parseSystem = `Você é um assistente que interpreta pedidos de alteração de landing pages.
Analise o texto e extraia TODAS as alterações solicitadas pelo cliente.
Responda APENAS em JSON válido, sem markdown, sem explicações fora do JSON.
Formato exato:
{
  "alteracoes": [
    {
      "de": "texto original que deve ser trocado (ou descrição do campo)",
      "para": "novo texto",
      "tipo": "substituicao_exata | novo_texto | remover",
      "contexto": "onde fica na página (ex: título principal, botão CTA, seção sobre nós)"
    }
  ]
}`;

        const textsPreview = texts.slice(0, 50).map((t,i) => `${i+1}. [${t.wt}] ${t.text.substring(0,80)}`).join('
');

        const parseMsg = `TEXTOS ATUAIS DA PÁGINA (para contexto):
${textsPreview}

ALTERAÇÕES SOLICITADAS PELO CLIENTE:
${alteracoesTexto}

Interprete e estruture todas as alterações em JSON.`;

        const parseResponse = await callClaude(parseSystem, parseMsg, 'claude-sonnet-4-20250514');

        let alteracoesParsed = [];
        try {
          const cleanJson = parseResponse.replace(/```json|```/g, '').trim();
          const parsed2 = JSON.parse(cleanJson);
          alteracoesParsed = parsed2.alteracoes || [];
        } catch(e) {
          throw new Error('Erro ao interpretar alterações: ' + e.message);
        }

        if (alteracoesParsed.length === 0) throw new Error('Nenhuma alteração identificada no texto');

        // Passo 2: Para cada alteração, encontrar o texto correspondente no JSON e aplicar
        const applySystem = `Você é um assistente que aplica alterações em textos de landing pages.
Para cada alteração, encontre o texto correspondente na lista e forneça o texto novo.
Responda APENAS em JSON válido:
{
  "aplicacoes": [
    {
      "indice": numero_do_item_na_lista_de_textos (0-based),
      "textoNovo": "novo texto a aplicar",
      "confianca": "alta | media | baixa",
      "motivo": "por que este campo"
    }
  ]
}`;

        const applyMsg = `ALTERAÇÕES A APLICAR:
${alteracoesParsed.map((a,i) => `${i+1}. DE: "${a.de}" PARA: "${a.para}" (${a.contexto})`).join('
')}

TEXTOS DISPONÍVEIS NA PÁGINA:
${texts.map((t,i) => `[${i}] [${t.wt}] "${t.text.substring(0,100)}"`).join('
')}

Para cada alteração, identifique qual índice da lista deve ser alterado e qual o novo texto.
Retorne APENAS as aplicações com alta ou média confiança.`;

        const applyResponse = await callClaude(applySystem, applyMsg, 'claude-sonnet-4-20250514');

        let aplicacoes = [];
        try {
          const cleanJson = applyResponse.replace(/```json|```/g, '').trim();
          aplicacoes = JSON.parse(cleanJson).aplicacoes || [];
        } catch(e) {
          throw new Error('Erro ao mapear alterações: ' + e.message);
        }

        // Passo 3: Aplicar as alterações no JSON
        const adaptedCopies = texts.map(t => t.text); // começa com originais
        aplicacoes.forEach(ap => {
          if (ap.indice >= 0 && ap.indice < texts.length && ap.textoNovo) {
            adaptedCopies[ap.indice] = ap.textoNovo;
          }
        });
